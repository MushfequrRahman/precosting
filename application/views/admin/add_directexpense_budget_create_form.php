...
foreach ($ul as $row) {
  $directexpenseitemid .= '<option value="' . $row["directexpenseitemid"] . '">' . $row["directexpenseitem"] . '</option>';
}
foreach ($ul1 as $row) {
  $directexpensetypeid .= '<option value="' . $row["directexpensetypeid"] . '">' . $row["directexpensetype"] . '</option>';
}
foreach ($ul2 as $row) {
...
foreach ($nl as $rown) {
  $nomiid .= '<option value="' . $rown["nomiid"] . '">' . $rown["nominame"] . '</option>';
}
?>
              ...
              <div class="col-md-12">
                <div class="box box-danger">
                  <div class="box-header with-border">
                    <h3 class="box-title">Direct Expense Budget Insert</h3>
                    <div class="row">
                    ...
                    <form role="form" name="insert_form" id="insert_form" autocomplete="off" method="post" enctype="multipart/form-data">
                      <div class="row">
                        <div class="col-md-12">
                          ...
                          <input type="text" class="form-control pd" name="debcdate" readonly id="pd" value="<?php echo date('d-m-Y'); ?>">
                        </div>
                      ...
                      <div id="AuGroup">
                        <div class="row">
                          <table class="table table-bordered" id="item_table1">
                            <thead>
                              <tr>
                                <th style="text-align:center;">Direct Expense Item</th>
                                <th style="text-align:center;">Direct Expense Type</th>
  ...
  <script>
    $(document).ready(function() {
      ...
      $(document).on('click', '.add', function() {
        ...
        var html = '';
        html += '<tr>';
        html += '<td><select name="directexpenseitemid[]" class="form-control directexpenseitemid" id="directexpenseitemid' + count + '"><option value="">Direct Expense Item</option><?php echo $directexpenseitemid; ?></select></td>';
        html += '<td><select name="directexpensetypeid[]" class="form-control directexpensetypeid" id="directexpensetypeid' + count + '"><option value="">Direct Expense Type</option><?php echo $directexpensetypeid; ?></select></td>';
      ...
      $('#insert_form').on('submit', function(event) {
        ...
        $('#directexpenseitemid').each(function() {
          var count = 1;
          if ($(this).val() == '') {
            error += '<p>Enter Direct Expense Item</p>';
            return false;
          }
          count = count + 1;
        });

        $('.directexpensetypeid').each(function() {
          var count = 1;
          if ($(this).val() == '') {
            error += '<p>Enter Direct Expense Type at ' + count + ' Row</p>';
            return false;
          }
          count = count + 1;
        });
        ...
        if (error == '') {
          $.ajax({
            url: "<?php echo base_url(); ?>Dashboard/add_directexpense_budget_insert",
            method: "get",
            data: form_data,
            success: function(data) {
              ...
              if (data == 'ok') {
                ...
                $('#error').html('<div class="alert alert-success">Direct Expense Received Details Saved</div>');
