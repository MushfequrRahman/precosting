...
class Admin extends CI_Model
{
	...
	{
		$sql = "UPDATE fabric_budget SET fabricitemid='$data[fabricitemid]',fabrictypeid='$data[fabrictypeid]',supplierid='$data[supplierid]',fborderqty='$data[orderqty]',cad='$data[cad]',allowance='$data[allowance]',puomid='$data[puomid]',rate='$data[rate]' WHERE fbid='$data[fbid]'";
		return $query = $this->db->query($sql);
	}
	...
	{
		$sql = "UPDATE trims_budget SET trimsitemid='$data[trimsitemid]',trimstypeid='$data[trimstypeid]',supplierid='$data[supplierid]',tborderqty='$data[orderqty]',cad='$data[cad]',allowance='$data[allowance]',puomid='$data[puomid]',rate='$data[rate]' WHERE tbid='$data[tbid]'";
		return $query = $this->db->query($sql);
	}
	...
	public function precost_update($buyerid, $styleid,$lc,$mlc,$accseasonid, $oqty, $fv,$cmdz,$dmlc,$finhdate,$prdate,$fsdate,$btp,$garmentstypeid,$pcid)
	{
		$finhdate = date("Y-m-d", strtotime($finhdate));
		$prdate = date("Y-m-d", strtotime($prdate));
		$fsdate = date("Y-m-d", strtotime($fsdate));
		$sql = "UPDATE precost_insert SET buyerid='$buyerid',styleid='$styleid',lc='$lc',mlc='$mlc',accseasonid='$accseasonid',oqty='$oqty',fv='$fv',cmdz='$cmdz',dmlc='$dmlc',finhdate='$finhdate',prdate='$prdate',fsdate='$fsdate',btp='$btp',garmentstypeid='$garmentstypeid'  WHERE pcid='$pcid'";
		return $query = $this->db->query($sql);
	}
